/*
 * test_muladd_depth_8192_4_ABC_plus_accD.cpp
 *
...
void test_muladd_depth_8192_4_ABC_plus_accD(double range_limit = 100.0, int run = 10){

	cout << "test_muladd_depth_8192_4_ABC_plus_accD(" << run << ")" << endl;
     ...
     * perform accD = D + D
     */
    Ciphertext encrypted_accD;
    evaluator.add(encrypted_D, encrypted_D, encrypted_accD);

    cout << "Result Acc 2D" << endl;
    print_ciphertext_info(encrypted_accD,context);

    check_chiphertext(&decryptor, &encoder, encrypted_accD, expected_accD);

    /*
     * perform run-2 times accD += D
     */
    for(int i=0; i<run-2;i++){
         ...
         * check scale
         */
        if(!check_operand_scale(encrypted_accD, encrypted_D)){

    		/*
    		 * fix the scale of accD by forcing the expected value
    		 */
    		encrypted_accD.scale() = pow(2.0,scale_exp);

    		cout << "Result accD fix scale" << endl;
    		print_ciphertext_info(encrypted_accD,context);

    		check_chiphertext(&decryptor, &encoder, encrypted_accD, expected_accD);
        }

    	evaluator.add_inplace(encrypted_accD, encrypted_D);

        for(int j=0;j<input_D.size();j++){
        	expected_accD[j] += input_D[j];
        }

        cout << "Result Acc " << i+3 << "D" << endl;
        print_ciphertext_info(encrypted_accD,context);
    ...
    /*
     * switch accD to the same prime as A*B*C
     */
    evaluator.mod_switch_to_inplace(encrypted_accD, encrypted_ABC.parms_id());

    cout << "accD mod switch" << endl;
    print_ciphertext_info(encrypted_accD,context);
     ...
     * check scale
     */
    if(!check_operand_scale(encrypted_ABC, encrypted_accD)){
        /*
    ...
    /*
     * perform addition ((A*B)*C) + accD
     */
    check_operand_scale(encrypted_ABC, encrypted_accD);

    Ciphertext encrypted_ABC_plus_accD;
    evaluator.add(encrypted_ABC, encrypted_accD, encrypted_ABC_plus_accD);

    cout << "Result ABC + accD" << endl;
    print_ciphertext_info(encrypted_ABC_plus_accD,context);

    vector<double> expected_ABC_plus_accC;
    for(int i=0;i<expected_ABC.size();i++){
    	expected_ABC_plus_accC.push_back(expected_ABC[i]+expected_accD[i]);
    }

    check_chiphertext(&decryptor, &encoder, encrypted_ABC_plus_accD, expected_ABC_plus_accC);
