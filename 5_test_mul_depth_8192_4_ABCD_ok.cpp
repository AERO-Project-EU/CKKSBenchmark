/*
 * test_mul_depth_8192_4_ABCD_ok.cpp
 *
...
void test_mul_depth_8192_4_ABCD_ok(double range_limit = 100.0){
	...
	cout << "test_mul_depth_8192_4_ABCD_ok()" << endl;
    ...
    vector<double> expected_CD;
    vector<double> expected_ABCD;
    for(int i=0;i<input_A.size();i++){
    	expected_AB.push_back(input_A[i]*input_B[i]);
    	expected_CD.push_back(input_C[i]*input_D[i]);
    	expected_ABCD.push_back(expected_AB[i]*expected_CD[i]);
    }
    ...
    /*
     * perform multiplication C*D
     */
    Ciphertext encrypted_CD;
    evaluator.multiply(encrypted_C, encrypted_D, encrypted_CD);

    cout << "Result CD" << endl;
    print_ciphertext_info(encrypted_CD,context);

    check_chiphertext(&decryptor, &encoder, encrypted_CD, expected_CD);

    cout << "--------------------------" << endl << endl;

    /*
     * relinearize C*D to return to size = 2
     */
    evaluator.relinearize_inplace(encrypted_CD,relin_keys);

    cout << "Result CD relin" << endl;
    print_ciphertext_info(encrypted_CD,context);

    check_chiphertext(&decryptor, &encoder, encrypted_CD, expected_CD);

    /*
     * rescale C*D to next prime in the chain
     */
    evaluator.rescale_to_next_inplace(encrypted_CD);

    cout << "Result CD rescale" << endl;
    print_ciphertext_info(encrypted_CD,context);
     ...
     * check scale
     */
    if(!check_operand_scale(encrypted_AB, encrypted_CD)){

		/*
		 * fix the scale of A*B and C*D by forcing the expected value
		 */
		encrypted_AB.scale() = pow(2.0,scale_exp);
		encrypted_CD.scale() = pow(2.0,scale_exp);
    ...
    evaluator.multiply(encrypted_AB, encrypted_CD, encrypted_ABCD);
